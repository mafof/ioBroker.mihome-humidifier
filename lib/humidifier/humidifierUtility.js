...
class HumidifierUtility extends RecursiveUtils {
    ...
    onStateChange(id, state) {
        id = id.split(/([0-9]\.)/)[2];
        let value = (id == "control.power" || id == "control.buzzer" || id == "control.child_lock" || id == "control.dry") ? (state.val) ? "on" : "off" : state.val;
        let command = "";
    ...
    sendCommandGettingAllStatesDevice() {
        return this.listenerState.sendCommand('get_prop', ["power", "mode", "led", "buzzer", "limit_hum", "depth", "speed", "temp", "humidity", "child_lock", "dry"]);
    }
